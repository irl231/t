    ...
    public class TweenLite extends Animation 
    {
            ...
            while (--i > -1)
            {
                ...
                if (!(((curTween === tween) || (curTween._gc)) || (curTween._paused)))
                {
                    if (curTween._timeline != tween._timeline)
                    {
                        ...
                        {
                            var _local_15:* = oCount++;
                            overlaps[_local_15] = curTween;
                        };
                    }
                    else
                    {
        ...
        protected function _initProps(target:Object, propLookup:Object, siblings:Array, overwrittenProps:Object):Boolean
        {
            ...
            for (p in vars)
            {
                ...
                {
                    if (((p in _plugins) && ((plugin = new (_plugins[p])())._onInitTween(target, val, this))))
                    {
                    ...
                    else
                    {
                        ...
                        this._firstPT.s = ((this._firstPT.f) ? target[(((p.indexOf("set")) || (!(("get" + p.substr(3)) in target))) ? p : ("get" + p.substr(3)))]() : Number(target[p]));
        ...
        override public function render(time:Number, suppressEvents:Boolean=false, force:Boolean=false):void
        {
            ...
            while (pt)
            {
                ...
                {
                    var _local_10:* = pt.t;
                    (_local_10[pt.p](((pt.c * this.ratio) + pt.s)));
                }
                else
                {
        ...
        {
            var i:int;
            var overwrittenProps:Object;
            var p:String;
            var pt:PropTween;
            var propLookup:Object;
            var changed:Boolean;
            var killProps:Object;
            var record:Boolean;
            if (vars === "all")
            {
                vars = null;
